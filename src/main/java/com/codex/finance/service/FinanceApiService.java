...
public class FinanceApiService {
    ...
    public ContractDtos.TransactionResponse updateTransaction(String userId, String id, ContractDtos.UpsertTransactionRequest request) {
        ...
        Object[] result = movementRepo.updateTransaction(movementUuid, userUuid,
                mapper.toUuid(request.accountId()), mapper.toUuid(request.transferAccountId()),
                mapper.toUuid(request.categoryId()), request.type(), request.amount(), request.description(),
                request.currency(), request.transactionDate(), request.notes());
        return mapper.mapToTransactionResponse(result);
    }
    ...
    public ContractDtos.RecurringPaymentResponse updateRecurringPayment(String userId, String id, ContractDtos.UpsertRecurringPaymentRequest request) {
        ...
        Object[] result = scheduledPaymentRepo.updateRecurringPayment(spUuid, userUuid, request.name(), request.amount(),
    ...
    public ContractDtos.InstallmentResponse createInstallment(String userId, ContractDtos.UpsertInstallmentRequest request) {
        ...
        installmentRepo.createInstallment(uuid, debtUuid, request.number(), request.amount(),
    ...
    public ContractDtos.InstallmentResponse updateInstallment(String userId, String id, ContractDtos.UpsertInstallmentRequest request) {
        ...
        installmentRepo.updateInstallment(uuid, installmentUuid, request.number(), request.amount(),
 ...
 // ==================== CREDIT CARD INSTALLMENT (Compra a meses) ====================
    public ContractDtos.InstallmentResponse createCreditCardInstallment(String userId, ContractDtos.UpsertInstallmentRequest request) {
        UUID uuid = UUID.fromString(userId);
        ...
        if (debtUuid == null && accountUuid != null) {
            ...
            Object[] newDebt = debtRepo.createDebt(
                uuid,
                debtName,
                request.originalPurchaseAmount() != null ? request.originalPurchaseAmount() : request.amount().multiply(BigDecimal.valueOf(request.number())),
                request.amount(),
                "monthly",
                request.dueDate().toString(),
                "Compra a meses con tarjeta"
            );
        ...
        installmentRepo.createCreditCardInstallment(
            ...
            request.amount(), 
            request.dueDate(),
            request.originalPurchaseAmount(), 
    ...
    public ContractDtos.FinancialGoalResponse createGoal(String userId, ContractDtos.UpsertFinancialGoalRequest request) {
        ...
        financialGoalRepo.createGoal(uuid, request.name(), request.targetAmount(),
                ...
                request.targetDate(), request.status() != null ? request.status() : "active");
    ...
    public ContractDtos.FinancialGoalResponse updateGoal(String userId, String id, ContractDtos.UpsertFinancialGoalRequest request) {
        ...
        financialGoalRepo.updateGoal(uuid, goalUuid, request.name(), request.targetAmount(),
                request.currentProgress(), request.targetDate(), request.status());
    ...
    public ContractDtos.BudgetResponse createBudget(String userId, ContractDtos.UpsertBudgetRequest request) {
        ...
        budgetRepo.createBudget(uuid, categoryUuid, request.period(), periodStart, periodEnd,
    ...
    public List<ContractDtos.CategoryStatResponse> categoryStats(String userId, String range, LocalDate from, LocalDate to, String accountId) {
        ...
        List<ContractDtos.CategoryStatResponse> raw = rows.stream()
                .map(r -> new ContractDtos.CategoryStatResponse(mapper.toString(r[0]), mapper.toString(r[1]), mapper.toBigDecimal(r[2]), BigDecimal.ZERO))
                .collect(Collectors.toList());
    ...
    public List<ContractDtos.BiweeklyScheduleResponse> getBiweeklySchedule(String userId) {
        ...
        BigDecimal firstTotal = firstPayments.stream().map(ContractDtos.BiweeklyPaymentItem::amount).reduce(BigDecimal.ZERO, BigDecimal::add);
        schedules.add(new ContractDtos.BiweeklyScheduleResponse("Primera quincena", firstHalfStart, firstHalfEnd, firstPayments, firstTotal, biweeklyIncome, biweeklyIncome.subtract(firstTotal)));
        
        List<ContractDtos.BiweeklyPaymentItem> secondPayments = getPaymentsInRange(uuid, secondHalfStart, secondHalfEnd);
        BigDecimal secondTotal = secondPayments.stream().map(ContractDtos.BiweeklyPaymentItem::amount).reduce(BigDecimal.ZERO, BigDecimal::add);
        schedules.add(new ContractDtos.BiweeklyScheduleResponse("Segunda quincena", secondHalfStart, secondHalfEnd, secondPayments, secondTotal, biweeklyIncome, biweeklyIncome.subtract(secondTotal)));
    ...
    public List<ContractDtos.MonthlyReportResponse> getMonthlyReports(String userId, int year) {
        ...
        for (Object[] row : rows) {
            ...
            reports.add(new ContractDtos.MonthlyReportResponse(month.toString(), income, expenses, savings, new ArrayList<>()));
        }
        return reports;
    }
