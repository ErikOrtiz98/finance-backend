...
public interface FinancialGoalRepository extends JpaRepository<FinancialGoal, UUID> {
	...
	void createGoal(@Param("userId") UUID userId, @Param("name") String name,
			@Param("targetAmount") BigDecimal targetAmount, @Param("currentProgress") BigDecimal currentProgress,
			@Param("targetDate") LocalDate targetDate, @Param("status") String status);

	@Modifying
	@Query(value = "UPDATE financial_goals SET " + "name = COALESCE(:name, name), "
			...
			+ "status = COALESCE(CAST(:status AS goal_status), status), " + "updated_at = NOW() "
			+ "WHERE id = :id AND user_id = :userId AND deleted_at IS NULL", nativeQuery = true)
	void updateGoal(@Param("userId") UUID userId, @Param("id") UUID id, @Param("name") String name,
			@Param("targetAmount") BigDecimal targetAmount, @Param("currentProgress") BigDecimal currentProgress,
			@Param("targetDate") LocalDate targetDate, @Param("status") String status);
