...
public interface MovementRepository extends JpaRepository<Movement, UUID> {
	...
	Object[] createTransaction(@Param("userId") UUID userId, @Param("accountId") UUID accountId,
			@Param("transferAccountId") UUID transferAccountId, @Param("categoryId") UUID categoryId,
			@Param("type") String type, @Param("amount") BigDecimal amount, @Param("description") String description,
			@Param("currency") String currency, @Param("transactionDate") LocalDate transactionDate,
	...
	@Query(value = "UPDATE movements SET account_id = :accountId, "
			...
			+ "CASE WHEN deleted_at IS NULL THEN 'synced' ELSE 'deleted' END AS syncStatus, "
			+ "COALESCE(row_version, 1) AS version", nativeQuery = true)
	Object[] updateTransaction(@Param("id") UUID id, @Param("userId") UUID userId, @Param("accountId") UUID accountId,
			@Param("transferAccountId") UUID transferAccountId, @Param("categoryId") UUID categoryId,
			@Param("type") String type, @Param("amount") BigDecimal amount, @Param("description") String description,
			@Param("currency") String currency, @Param("transactionDate") LocalDate transactionDate,
			@Param("notes") String notes);
	...
	@Query(value = "SELECT COUNT(1) FROM movements WHERE id = :id AND user_id = :userId", nativeQuery = true)
	...
	Object[] getSummary(@Param("userId") UUID userId, @Param("from") LocalDate from, @Param("to") LocalDate to,
	...
	@Query(value = "SELECT c.id AS categoryId, c.name AS categoryName, "
			...
			+ "WHERE c.user_id = :userId AND c.deleted_at IS NULL "
			+ "GROUP BY c.id, c.name ORDER BY amount DESC", nativeQuery = true)
	List<Object[]> getCategoryStatsByDateRange(@Param("userId") UUID userId, @Param("from") LocalDate from,
	...
	Object[] getSummaryFull(@Param("userId") UUID userId, @Param("from") LocalDate from, @Param("to") LocalDate to,
			@Param("accountId") UUID accountId);
	...
	@Query(value = "SELECT DATE_TRUNC('month', movement_date) as month, "
			...
			+ "FROM movements WHERE user_id = :userId AND deleted_at IS NULL " + "AND movement_date >= :startDate "
			+ "GROUP BY DATE_TRUNC('month', movement_date) " + "ORDER BY month DESC", nativeQuery = true)
	List<Object[]> getMonthlyReport(@Param("userId") UUID userId, @Param("startDate") LocalDate startDate);
	...
	@Query(value = "SELECT COALESCE(SUM(CASE WHEN m.movement_type = 'income' THEN m.amount ELSE 0 END), 0) as total_income, "
			...
			+ "COALESCE(SUM(CASE WHEN m.movement_type = 'payment' THEN m.amount ELSE 0 END), 0) as fixed_payments "
			+ "FROM movements m " + "WHERE m.user_id = :userId "
			+ "AND m.movement_date BETWEEN :startDate AND :endDate " + "AND m.deleted_at IS NULL "
	...
	Object[] getSummaryByDateRangeAndAccount(@Param("userId") UUID userId, @Param("startDate") LocalDate startDate,
			@Param("endDate") LocalDate endDate, @Param("accountId") UUID accountId);
}