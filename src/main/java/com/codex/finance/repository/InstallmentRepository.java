...
public interface InstallmentRepository extends JpaRepository<Installment, UUID> {
	...
	@Query(value = "SELECT i.id, i.debt_id, i.number, i.amount, i.due_date, i.paid, i.paid_at, "
			...
			+ "AND i.deleted_at IS NULL " + "ORDER BY i.due_date ASC " + "LIMIT :limit", nativeQuery = true)
	...
	@Query(value = "SELECT i.id, i.debt_id, i.number, i.amount, i.due_date, i.paid, i.paid_at, "
			...
			+ "CASE WHEN i.deleted_at IS NULL THEN 'synced' ELSE 'deleted' END AS syncStatus, "
			+ "COALESCE(i.row_version, 1) AS version " + "FROM installments i "
			+ "WHERE i.user_id = :userId AND i.account_id = :accountId " + "AND i.deleted_at IS NULL "
	...
	void createInstallment(@Param("userId") UUID userId, @Param("debtId") UUID debtId, @Param("number") Integer number,
			@Param("amount") BigDecimal amount, @Param("dueDate") LocalDate dueDate, @Param("paid") Boolean paid);

	@Modifying
	@Query(value = "INSERT INTO installments (id, debt_id, account_id, user_id, number, amount, due_date, "
			+ "original_purchase_amount, interest_rate, paid, created_at, updated_at, row_version) "
			+ "VALUES (gen_random_uuid(), :debtId, :accountId, :userId, :number, :amount, :dueDate, "
			+ ":originalPurchaseAmount, :interestRate, :paid, NOW(), NOW(), 1)", nativeQuery = true)
	void createCreditCardInstallment(@Param("userId") UUID userId, @Param("debtId") UUID debtId,
			@Param("accountId") UUID accountId, @Param("number") Integer number, @Param("amount") BigDecimal amount,
			@Param("dueDate") LocalDate dueDate, @Param("originalPurchaseAmount") BigDecimal originalPurchaseAmount,
			@Param("interestRate") BigDecimal interestRate, @Param("paid") Boolean paid);

	@Modifying
	@Query(value = "UPDATE installments SET " + "number = COALESCE(:number, number), "
			+ "amount = COALESCE(:amount, amount), " + "due_date = COALESCE(:dueDate, due_date), "
			+ "paid = COALESCE(:paid, paid), " + "updated_at = NOW(), " + "row_version = row_version + 1 "
			+ "WHERE id = :id AND user_id = :userId AND deleted_at IS NULL", nativeQuery = true)
	void updateInstallment(@Param("userId") UUID userId, @Param("id") UUID id, @Param("number") Integer number,
			@Param("amount") BigDecimal amount, @Param("dueDate") LocalDate dueDate, @Param("paid") Boolean paid);
