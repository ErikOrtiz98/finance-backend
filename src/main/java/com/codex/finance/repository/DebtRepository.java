...
public interface DebtRepository extends JpaRepository<Debt, UUID> {
    ...
    Object[] createDebt(@Param("userId") UUID userId, @Param("name") String name,
                       ...
                       @Param("nextDueDate") String nextDueDate, @Param("notes") String notes);
    ...
    @Query(value = "UPDATE debts SET name = :name, remaining_balance = :principalBalance, " +
           ...
           "CASE WHEN deleted_at IS NULL THEN 'synced' ELSE 'deleted' END AS syncStatus, " +
           "COALESCE(row_version, 1) AS version", nativeQuery = true)
    Object[] updateDebt(@Param("id") UUID id, @Param("userId") UUID userId,
                       @Param("name") String name, @Param("principalBalance") BigDecimal principalBalance,
                       @Param("installment") BigDecimal installment,
                       @Param("frequency") String frequency,
                       @Param("nextDueDate") String nextDueDate, @Param("notes") String notes);
    ...
    @Query(value = "SELECT COUNT(1) FROM debts WHERE id = :id AND user_id = :userId", 
           nativeQuery = true)
