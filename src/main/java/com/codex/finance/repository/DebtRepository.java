...
public interface DebtRepository extends JpaRepository<Debt, UUID> {
    ...
    @Query(value = "INSERT INTO debts (user_id, name, debt_type, original_amount, remaining_balance, "
           ...
           + "remaining_balance AS remainingBalance, "  // <-- CORREGIDO: remaining_balance
           + "original_amount AS principalBalance, "     // <-- CORREGIDO: original_amount
           + "fixed_payment AS installment, "
    ...
    @Modifying
    @Query(value = "UPDATE debts SET name = :name, remaining_balance = :remainingBalance, "
           ...
           + "remaining_balance AS remainingBalance, "   // <-- CORREGIDO
           + "original_amount AS principalBalance, "      // <-- CORREGIDO
           + "fixed_payment AS installment, "
           + "COALESCE(metadata->>'frequency', 'monthly') AS frequency, "
           + "COALESCE((metadata->>'nextDueDate')::date, NULL) AS nextDueDate, "
           + "COALESCE(metadata->>'notes', '') AS notes, created_at AS createdAt, "
    ...
    Object[] updateDebt(@Param("id") UUID id, @Param("userId") UUID userId, @Param("name") String name,
            @Param("remainingBalance") BigDecimal remainingBalance,   // <-- CORREGIDO: usar remainingBalance
            @Param("installment") BigDecimal installment,
            @Param("frequency") String frequency, @Param("nextDueDate") String nextDueDate,
            @Param("notes") String notes);
    ...
    // NUEVO MÉTODO: Para deducir solo el remaining_balance al pagar una partialidad
    @Modifying
    @Query(value = "UPDATE debts SET remaining_balance = remaining_balance - :amount, updated_at = NOW() "
           + "WHERE id = :debtId AND user_id = :userId AND deleted_at IS NULL", nativeQuery = true)
