...
public interface ProfileRepository extends JpaRepository<Profile, UUID> {
	...
	@Query(value = "INSERT INTO profiles (user_id, full_name, currency_code, settings, preferences, "
			...
			+ "VALUES (:userId, :displayName, :currency, CAST(:settings AS jsonb), "
			+ "COALESCE((SELECT preferences FROM profiles WHERE user_id = :userId2), CAST('{}' AS jsonb)), "
			+ "NOW(), NOW(), 1) " + "ON CONFLICT (user_id) DO UPDATE SET " + "full_name = EXCLUDED.full_name, "
			+ "currency_code = EXCLUDED.currency_code, " + "settings = EXCLUDED.settings, " + "updated_at = NOW(), "
			...
			+ "COALESCE(settings->>'payCycle', 'monthly') AS payCycle, " + "settings->'payDays' AS payDays, "
			+ "COALESCE((settings->>'monthlyIncome')::DECIMAL, 0) AS monthlyIncome, "
	...
	Object[] upsertProfile(@Param("userId") UUID userId, @Param("displayName") String displayName,
			@Param("currency") String currency, @Param("settings") String settings, @Param("userId2") UUID userId2);

	@Query(value = "SELECT COALESCE(currency_code, 'MXN') FROM profiles WHERE user_id = :userId", nativeQuery = true)
