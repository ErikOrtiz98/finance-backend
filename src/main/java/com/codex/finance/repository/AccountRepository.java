...
public interface AccountRepository extends JpaRepository<Account, UUID> {
	...
	Object[] createAccount(@Param("userId") UUID userId, @Param("name") String name, @Param("type") String type,
			...
			@Param("dueDay") Integer dueDay, @Param("active") Boolean active, @Param("currency") String currency);
	...
	@Query(value = "UPDATE accounts SET name = :name, "
			...
			+ "CASE WHEN deleted_at IS NULL THEN 'synced' ELSE 'deleted' END AS syncStatus, "
			+ "COALESCE(row_version, 1) AS version", nativeQuery = true)
	Object[] updateAccount(@Param("id") UUID id, @Param("userId") UUID userId, @Param("name") String name,
			@Param("type") String type, @Param("institution") String institution, @Param("balance") BigDecimal balance,
			@Param("creditLimit") BigDecimal creditLimit, @Param("closingDay") Integer closingDay,
	...
	@Query(value = "SELECT COUNT(1) FROM accounts WHERE id = :id AND user_id = :userId", nativeQuery = true)
